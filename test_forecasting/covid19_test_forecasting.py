"""Evaluate all Prophet, SARIMA, and LSTM configurations on the COVID-19 test set."""
...
DATA_PKL_PATH = os.path.join("data", "covid19", "covid19_dataset.pkl")
NON_DATE_COLS = ["country", "type", "Lat", "Long"]
SERIES_MAP_PKL = "covid19_series_to_id.pkl"
TEST_HORIZON = 14

PROPHET_TIMEOUT_SEC = 180
PROPHET_COMB_MAP_PKL = "covid19_comb_to_id.pkl"
PROPHET_SMAPE_TEST_PKL = "covid19_smape_matrix_test.pkl"
PROPHET_FORECAST_TEST_PKL = "covid19_forecasting_test.pkl"
PROPHET_METRIC_PERIOD = 1

SARIMA_TIMEOUT_SEC = 180
SARIMA_SEASONAL_PERIOD = 7
SARIMA_COMB_MAP_PKL = "covid19_comb_to_id_sarima.pkl"
SARIMA_SMAPE_TEST_PKL = "covid19_smape_matrix_test_sarima.pkl"
SARIMA_FORECAST_TEST_PKL = "covid19_forecasting_test_sarima.pkl"
SARIMA_METRIC_PERIOD = 7
...
LSTM_COMB_MAP_PKL = "covid19_comb_to_id_lstm.pkl"
LSTM_SMAPE_TEST_PKL = "covid19_smape_matrix_test_lstm.pkl"
LSTM_FORECAST_TEST_PKL = "covid19_forecasting_test_lstm.pkl"
LSTM_METRIC_PERIOD = 7


def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate all Prophet, SARIMA, and LSTM configurations on the COVID-19 test set."
    )
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        ...
        help="Directory shared with grid_search/covid19_grid_search.py: combination "
...
def load_data():
    """Load the COVID-19 series and identify the ordered date columns."""
    ...
    date_columns = sorted(parsed_dates, key=parsed_dates.get)
    dates = np.array([parsed_dates[column] for column in date_columns])

    for column in date_columns:
        data[column] = pd.to_numeric(data[column], errors="coerce")

    data = data.dropna(subset=["country", "type"] + date_columns)
    return data, date_columns, dates


def get_series_values(data, date_columns, country, series_type):
    row = data[(data["country"] == country) & (data["type"] == series_type)].iloc[0]
    return row[date_columns].astype(float).values
...
def run_prophet_test(data, date_columns, dates, series_map):
    ...
    print(
        f"[INFO] Starting COVID-19 Prophet test run. Already done: "
    ...
    for _, series in series_map.iterrows():
        ...
        y_all = get_series_values(data, date_columns, country, series_type)
        ...
        for _, combination in combo_map.iterrows():
            ...
            try:
                predictions = run_with_timeout(
                    prophet_fit_predict,
                    args=(train_data, test_data, params),
        ...
        print(
            ...
            f"(series_id={series_id}, country={country}, type={series_type})",
            flush=True,
        )
...
def run_sarima_test(data, date_columns, series_map):
    ...
    print(
        f"[INFO] Starting COVID-19 SARIMA test run. Already done: "
        f"{completed_count}/{total_series}",
        flush=True,
    )
    ...
    for _, series in series_map.iterrows():
        ...
        for _, combination in combo_map.iterrows():
            ...
            try:
                predictions = run_with_timeout(
                    sarima_fit_predict,
                    args=(y_train, TEST_HORIZON, params),
...
def run_lstm_test(data, date_columns, dates, series_map):
    ...
    print(
        f"[INFO] Starting COVID-19 LSTM test run. Already done: "
        f"{completed_count}/{total_series}",
        flush=True,
    )
    ...
    time_features = build_time_features(dates)
...
def main():
    global DATA_PKL_PATH, SERIES_MAP_PKL
    ...
    SERIES_MAP_PKL = os.path.join(args.output_dir, "covid19_series_to_id.pkl")

    PROPHET_COMB_MAP_PKL = os.path.join(args.output_dir, "covid19_comb_to_id.pkl")
    PROPHET_SMAPE_TEST_PKL = os.path.join(args.output_dir, "covid19_smape_matrix_test.pkl")
    PROPHET_FORECAST_TEST_PKL = os.path.join(args.output_dir, "covid19_forecasting_test.pkl")

    SARIMA_COMB_MAP_PKL = os.path.join(args.output_dir, "covid19_comb_to_id_sarima.pkl")
    SARIMA_SMAPE_TEST_PKL = os.path.join(args.output_dir, "covid19_smape_matrix_test_sarima.pkl")
    SARIMA_FORECAST_TEST_PKL = os.path.join(args.output_dir, "covid19_forecasting_test_sarima.pkl")

    LSTM_COMB_MAP_PKL = os.path.join(args.output_dir, "covid19_comb_to_id_lstm.pkl")
    LSTM_SMAPE_TEST_PKL = os.path.join(args.output_dir, "covid19_smape_matrix_test_lstm.pkl")
    LSTM_FORECAST_TEST_PKL = os.path.join(args.output_dir, "covid19_forecasting_test_lstm.pkl")

    data, date_columns, dates = load_data()
    series_map = load_required_table(SERIES_MAP_PKL, "series_id")
    run_prophet_test(data, date_columns, dates, series_map)
    run_sarima_test(data, date_columns, series_map)
    run_lstm_test(data, date_columns, dates, series_map)
