"""Run Prophet, SARIMA, and LSTM grid searches for the COVID-19 dataset."""
...
PKL_PATH = os.path.join("data", "covid19", "covid19_dataset.pkl")
...
PROPHET_SMAPE_MATRIX_PKL = "covid19_smape_matrix.pkl"
...
SARIMA_SMAPE_MATRIX_PKL = "covid19_smape_matrix_sarima.pkl"
...
LSTM_SMAPE_MATRIX_PKL = "covid19_smape_matrix_lstm.pkl"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Run Prophet, SARIMA, and LSTM grid searches for the COVID-19 dataset."
    )
    parser.add_argument(
        "--data-dir",
        default=os.path.join("data", "covid19"),
        help="Directory containing covid19_dataset.pkl "
        "(produced by data_preparation/covid19/create_dataset.py).",
    )
    parser.add_argument(
        "--output-dir",
        default=os.path.join("outputs", "covid19"),
        help="Directory to write validation grid-search matrices and "
        "combination maps to. test_forecasting/covid19_test_forecasting.py "
...
def load_data():
    """Load the COVID-19 series and identify the date columns."""
    ...
    for column in data.columns:
        ...
        if not pd.isna(parsed_date):
            date_to_column[column] = parsed_date

    date_columns = sorted(date_to_column, key=date_to_column.get)
    dates = np.asarray([date_to_column[column] for column in date_columns])
...
def build_series_map(data):
    """Assign a series ID to each country and observation type."""
    series = data[["country", "type"]].sort_values(["country", "type"])
    records = [
        {
            "series_id": series_id,
            "country": row.country,
        ...
        for series_id, row in enumerate(series.itertuples(index=False), start=1)
    ]
...
def get_series_values(data, date_columns, country, series_type):
    ...
    return series_data.iloc[0][date_columns].astype(float).values
...
def run_prophet_grid_search(data, date_columns, dates, series_map):
    ...
    print(
        f"[INFO] Starting COVID-19 Prophet grid search: "
    ...
    for row in series_map.itertuples(index=False):
        ...
        values = get_series_values(data, date_columns, row.country, row.type)
        n_total = len(values)
        ...
        if n_total <= TEST_HORIZON:
            ...
            print(
                ...
                f"(skipped: too short, series_id={series_id}, "
        ...
        if split_index <= 0 or split_index >= n_total:
            ...
            print(
                f"[INFO] Prophet {completed_count}/{total_series} "
                f"(skipped: bad split, series_id={series_id}, "
                f"country={row.country}, type={row.type})",
                flush=True,
            )
            continue
        ...
        if not splits:
            ...
            print(
                f"[INFO] Prophet {completed_count}/{total_series} "
                f"(skipped: no CV splits, series_id={series_id}, "
                f"country={row.country}, type={row.type})",
                flush=True,
            )
            continue
        ...
        print(
            ...
            f"(last series_id={series_id}, country={row.country}, type={row.type})",
            flush=True,
        )
...
def run_sarima_grid_search(data, date_columns, series_map):
    ...
    print(
        f"[INFO] Starting COVID-19 SARIMA grid search: "
        f"{len(combo_ids)} combinations.",
        flush=True,
    )
...
def run_lstm_grid_search(data, date_columns, dates, series_map):
    ...
    print(f"[INFO] Starting COVID-19 LSTM grid search: {len(combo_ids)} combinations.", flush=True)
...
def main():
    global PKL_PATH, SERIES_MAP_PKL, PROPHET_COMB_MAP_PKL, PROPHET_SMAPE_MATRIX_PKL
    global SARIMA_COMB_MAP_PKL, SARIMA_SMAPE_MATRIX_PKL, LSTM_COMB_MAP_PKL, LSTM_SMAPE_MATRIX_PKL

    args = parse_args()
    os.makedirs(args.output_dir, exist_ok=True)

    PKL_PATH = os.path.join(args.data_dir, "covid19_dataset.pkl")
    ...
    PROPHET_SMAPE_MATRIX_PKL = os.path.join(args.output_dir, "covid19_smape_matrix.pkl")
    SARIMA_COMB_MAP_PKL = os.path.join(args.output_dir, "covid19_comb_to_id_sarima.pkl")
    SARIMA_SMAPE_MATRIX_PKL = os.path.join(args.output_dir, "covid19_smape_matrix_sarima.pkl")
    LSTM_COMB_MAP_PKL = os.path.join(args.output_dir, "covid19_comb_to_id_lstm.pkl")
    LSTM_SMAPE_MATRIX_PKL = os.path.join(args.output_dir, "covid19_smape_matrix_lstm.pkl")

    data, date_columns, dates = load_data()
    series_map = build_series_map(data)

    run_prophet_grid_search(data, date_columns, dates, series_map)
    run_sarima_grid_search(data, date_columns, series_map)
    run_lstm_grid_search(data, date_columns, dates, series_map)
